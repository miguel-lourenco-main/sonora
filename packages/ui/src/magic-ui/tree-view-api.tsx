        ...
        <div className={cn("size-full", className)}>
          ...
          >
            <AccordionPrimitive.Root
              ...
              onValueChange={(value) =>{
                if(value[0]) setExpendedItems((prev) => [...(prev ?? []), (value[0] ?? "")])
              }}
              dir={dir as Direction}
            >
              {children}
...
>(
  (
    ...
    useEffect(() => {
      if(expand && openFirst && setOpenFirst && setExpendedItems){
      ...
      >
        ...
        >
          {expendedItems?.includes(value)
            ? openIcon ?? <FolderOpenIcon className="h-4 w-4" />
            : closeIcon ?? <FolderIcon className="h-4 w-4" />}
          <span>{<Skeleton className="w-[16rem] h-4"/>}</span>
        </AccordionPrimitive.Trigger>
        <AccordionPrimitive.Content className="text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative overflow-hidden h-full">
...
>(
  (
    ...
    const { direction, selectedId, selectItem, onFileClick } = useTree();
...
export { Tree, Folder, File, CollapseButton, FolderSkeleton, FileSkeleton, type TreeViewElement };
