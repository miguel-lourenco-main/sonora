...
async function run() {
  const result = await polydoc.runs.runsList();
...
import { runsRunsList } from "polydoc/funcs/runsRunsList.js";
...
async function run() {
  const res = await runsRunsList(polydoc);
...
async function run() {
  const result = await polydoc.runs.runsCreate({
...
```typescript
import { PolydocCore } from "polydoc/core.js";
import { runsRunsCreate } from "polydoc/funcs/runsRunsCreate.js";
...
async function run() {
  const res = await runsRunsCreate(polydoc, {
    inputFileId: "d663dc1e-405a-4327-9704-e6034b018fa2",
    targetLanguage: "<value>",
  });
