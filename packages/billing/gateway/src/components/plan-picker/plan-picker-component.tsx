...
export function PlanPickerComponent(
  ...
  useEffect(() => {
    ...
    if(props.currentSubscriptionVariantId !== undefined && selectedPlan?.lineItems[0]?.id === props.currentSubscriptionVariantId){
      setIsCurrentPlanSelected(true);
    }else {
      setIsCurrentPlanSelected(false);
    }
    
  }, [props.currentSubscriptionVariantId, selectedPlan, isCurrentPlanSelected]);
