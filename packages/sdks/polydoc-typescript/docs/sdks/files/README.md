...
async function run() {
  const result = await polydoc.files.filesDownload({
...
```typescript
import { PolydocCore } from "polydoc/core.js";
import { filesFilesDownload } from "polydoc/funcs/filesFilesDownload.js";
...
async function run() {
  const res = await filesFilesDownload(polydoc, {
    fileId: "4a176118-b862-4d43-860a-6e4c9c893bde",
  });
